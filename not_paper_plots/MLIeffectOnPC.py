...
def plot():
    ...
    results = h5py.File(results_path("results_stim_on_MFs_condMLI_1.hdf5"), "r")

    resultsGABAZ = h5py.File(results_path("results_stim_on_MFs_condMLI_2.hdf5"), "r")
    ...
    maxSpikes=max(numSpikesStim.values())
    #Activity = {id: v if len(v) else 0 for id, v in numSpikesStim.items() if id not in cut_off}
    ActivityNorm = {id: avg(inv(v)) if len(v) else 0 for id, v in pc_isis.items() if id not in cut_off}
    ...
    maxSpikes=max(numSpikesStim.values())
    #Activity = {id: v if len(v) else 0 for id, v in numSpikesStim.items() if id not in cut_off}
    ActivityGABAZNorm = {id: avg(inv(v)) if len(v) else 0 for id, v in pc_isis.items() if id not in cut_off}

    CenterSurr = {id: ActivityNorm[id] - ActivityGABAZNorm[id] for id, v in ActivityNorm.items() if id not in cut_off}
    # pixel  x z
