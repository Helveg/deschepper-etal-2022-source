...
def plot(net_path=None):
    ...
    cpos = {selection_mf: [] for selection_mf in [240, 241, 274, 186]}
    print(cpos)

    cs = scaffold.get_connectivity_set("mossy_to_glomerulus")
    #mf_glom = scaffold.cell_connections_by_tag["mossy_to_glomerulus"]
    ...
    for mf_id in list(data.keys()):
        if mf_id in [240, 241, 274, 186]:
            for a in np.array(data[mf_id]):
                cpos[mf_id].append(pos_map[int(a)])

    centroids = np.array([np.mean(cp, axis=0) if len(cp) else np.nan for cp in cpos.values()])
    print(centroids)
