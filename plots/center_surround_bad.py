...
def plot(path_control=None, path_gaba=None, network=None):
    ...
    # List of center surround INDUCING formulas:
    #
    # See `center_surround_debunk.py` to understand how each formula incorrectly
    # introduces mexican hat shapes in complete absence of center surround features.
    E = control
    I = gabazine - control
    En = E / np.max(E)
    In = I / np.max(I)
    ...
    Bn = En - In
    nB = (nc - (ng - nc)) / nc
    nB_corr = (nc - (ng - nc)) / (nc + 1)
    nB_diff = (nc - (ng - nc))
    plots = {
        "classic_error": B,
        "0div_corr_error": B_corr,
        "normalized_error": Bn,
        "prenormalized_classic_error": nB,
        "prenormalized_0div_corr_error": nB_corr,
        "prenormalized_diff_error": nB_diff,
    }
    figs = {}
