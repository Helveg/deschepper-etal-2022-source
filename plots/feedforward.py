...
def plot_average_all():
    ...
    corr_fig = make_subplots(rows=1, cols=3, y_title="Current (nA)", x_title="Time (ms)")
    corr_fig = corr_fig.add_trace(go.Scatter(x=corr_x, y=corr_y), row=1, col=3)
    max_x = np.argmax(corr_y)
    max_y = corr_y[max_x]
    corr_fig.add_annotation(x=corr_x[max_x], y=max_y, text=round(corr_x[max_x], 2), standoff=3, row=1, col=3)
    corr_fig.add_trace(go.Scatter(x=time, y=avg_ampa_current), row=1, col=1)
    corr_fig.add_trace(go.Scatter(x=time, y=avg_gaba_current), row=1, col=1)
    corr_fig.add_trace(go.Scatter(x=time, y=norm_ampa), row=1, col=2)
    corr_fig.add_trace(go.Scatter(x=time, y=norm_gaba), row=1, col=2)
    corr_fig.update_xaxes(range=[350, 450])
    corr_fig.update_xaxes(range=[-50, 50], row=1, col=3)
