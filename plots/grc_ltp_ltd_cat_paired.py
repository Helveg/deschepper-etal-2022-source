...
def make_psi():
    ...
    # go.Figure([
    #     go.Scatter(x=c[:, 0], y=c[:, 1], name="og"),
    #     go.Scatter(x=np.linspace(0, 0.00105, 1000), y=f_intrp(np.linspace(0, 0.00105, 1000)), name="interpolated")
    # ], layout_title_text="psicheck").show()
    return f_intrp

psi = make_psi()
print("psicheck", psi(0), psi(0.00000001))
...
def plot(path=None, network=None):
    ...
    surfaces = dict(
        ...
        ltp_cat=dict(
            files=glob(os.path.join(path, "*.hdf5")),
            data=lambda f: analyze_calcium(ps_grc.identifiers, f["recorders/ions/ca/"], stim_start, stim_end + after_effects, soma=False),
            bin_reduce=lambda v: sum(1 for x in v if x > 0) / len(v),
            reduce=run_avg,
            smoothen=True,
        ),
        ltd_cat=dict(
            files=glob(os.path.join(path, "*.hdf5")),
            data=lambda f: analyze_calcium(ps_grc.identifiers, f["recorders/ions/ca/"], stim_start, stim_end + after_effects, soma=False, inv=True),
            bin_reduce=lambda v: sum(1 for x in v if x > 0.008) / len(v),
            reduce=run_avg,
            smoothen=True,
        ),
    )
    ...
    if not frozen:
        ...
        with open("calcium.pickle", "wb") as f:
            pickle.dump(surfaces, f)
    else:
        with open("calcium.pickle", "rb") as f:
    ...
    plots = {"ltd_cat": plots["ltd_cat"], "ltp_cat": plots["ltp_cat"]}
    figs = {}
    ...
    for name, z in plots.items():
        ...
        kw = {}
        if name == "ltp_cat":
            fig_overlap.add_trace(go.Surface(z=z, name=name, opacity=0.7, colorscale="Viridis"))
        else:
            fig_overlap.add_trace(go.Surface(z=-z, name=name, opacity=0.8, reversescale=True))
        ...
        fig_overlap.update_layout(showlegend=True)
    return fig_overlap
